...
/**
 * MeshGradient — 4 анімованих radial-gradient blob-и у Win2K палітрі.
 *
 * Архітектура:
 *   - Один wrapper motion.div з useSpring для swipe реакції (зсуває всі blob-и разом)
 *   - Кожен blob — окремий motion.div з незалежними drift keyframes у vw/vh
 *
 * Props:
 *   swipeDirection: 'left' | 'right' | null — напрямок останнього свайпу
 */
export default function MeshGradient({ swipeDirection }) {
  const swipeX = useSpring(0, { stiffness: 60, damping: 20 })

  React.useEffect(() => {
    if (swipeDirection === 'left') {
      swipeX.set(-35)
      setTimeout(() => swipeX.set(0), 600)
    } else if (swipeDirection === 'right') {
      swipeX.set(35)
      setTimeout(() => swipeX.set(0), 600)
    }
  }, [swipeDirection, swipeX])
  ...
  return (
    ...
    >
      {/* Swipe wrapper — зсуває всі blob-и разом при свайпі */}
      <motion.div
        style={{
          position: 'absolute',
          inset: 0,
